...
export async function main(ns: NS) {
  ...
  const cities: (CityName | `${CityName}`)[] = ['Aevum', 'Chongqing', 'New Tokyo', 'Ishima', 'Volhaven', 'Sector-12'];
    ...
    try {
      for (let i = 0; i < 9; i++) {
        ...
        c.setAutoJobAssignment(
          ...
          cities[0],
          jobs[0],
          Math.floor(c.getOffice(tobaccoName, cities[0]).employees / 3.5)
        );
        c.setAutoJobAssignment(
          tobaccoName,
          cities[0],
        ...
        c.setAutoJobAssignment(
          ...
          cities[0],
          jobs[2],
          Math.floor((0.5 * c.getOffice(tobaccoName, cities[0]).employees) / 3.5)
        );
        c.setAutoJobAssignment(
          tobaccoName,
          cities[0],
          jobs[3],
          Math.ceil(c.getOffice(tobaccoName, cities[0]).employees / 3.5)
        );
      }
    } catch {}
