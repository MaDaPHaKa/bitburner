...
export async function main(ns: NS) {
  ...
  const RAM_PER_THREAD = ns.getScriptRam(CHEESY_SERVER_WEAKEN_SCRIPT);
  ...
  while (true) {
    ...
    if (
      ns.getHackingLevel() < GOOD_HACK_LEVEL &&
      ns.serverExists(PRIME_TARGET) &&
      ns.hasRootAccess(PRIME_TARGET) &&
      ns.getServerMaxMoney(PRIME_TARGET) > 1
    )
      earlyGameCheck = false;

    for (let target of TARGETS) {
      // ...we stick with THE BEST one. Adjust the criteria to your liking.
      if (earlyGameCheck) target = PRIME_TARGET;
      ...
      for (let server of servers) {
        ...
        let threadsToUse = Math.floor(
          (ns.getServerMaxRam(server) -
            ns.getServerUsedRam(server) -
            (server == "home" ? RESERVED_HOME_RAM : 0)) /
            RAM_PER_THREAD
        );
        ...
        if (server == "home") {
          ...
          GW_THREADS =
            ns.growthAnalyzeSecurity(1, target, S_CORES) / WEAKEN_PER_THREAD;
        }
        ...
        if (currentSecLevel > S_MIN_SEC_LEVEL) {
          ...
          if (currentSecLevel - REDUCED_SEC_LEVEL < S_MIN_SEC_LEVEL) {
            threadsToUse = Math.ceil(
              (currentSecLevel - S_MIN_SEC_LEVEL) / WEAKEN_PER_THREAD
            );
            currentSecLevel = S_MIN_SEC_LEVEL;
          } else currentSecLevel -= REDUCED_SEC_LEVEL;
          ns.exec(
            CHEESY_SERVER_WEAKEN_SCRIPT,
            server,
            threadsToUse,
        ...
        // Grow the server to the maximum money
          ...
          availableMoney < S_MAX_MONEY &&
          (requiredGrowThreads != 0 || !growthCalculated)
        ) {
          if (!growthCalculated) {
            requiredGrowThreads = Math.ceil(
              ns.growthAnalyze(target, S_MAX_MONEY / availableMoney, S_CORES)
            );
          ...
          currentSecLevel += ns.growthAnalyzeSecurity(
            threadsToUse,
            target,
            S_CORES
          );
        ...
        // Fully prepped - Let's do batching
          ...
          while (true) {
            ...
            growThreads = Math.ceil(
              ns.growthAnalyze(
                target,
                1 / (1 - Math.min(0.99, HACK_MONEY_PERCENTAGE * hackThreads)),
                S_CORES
              )
            );
